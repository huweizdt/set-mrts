...
BPTask::BPTask(uint id, ulong wcet, ulong period, ulong deadline,int linkID,bool status) {
  this->id = id;
  this->index = id;
  this->wcet = wcet;
  this->status = status;
...
BPTask::BPTask(uint id, ulong wcet, ulong period, ulong deadline, uint priority,int linkID,bool status) {
  this->id = id;
  this->linkID = linkID;
  this->index = id;
  this->wcet = wcet;
...
void BPTask::set_linkID(uint linkID)  { this->linkID=linkID; }
void BPTask::set_status(bool status) { this->status=status; }
...
void BPTaskSet::task_copy(int size)
{
   ...
   foreach(tasks,ti)
   {
    ...
    break;
   bool status=true;
    BPTask temp_task(size+ti->get_id(),ti->get_wcet(),ti->get_period(),ti->get_deadline(),ti->get_id(),status);
    ti->set_linkID(size+ti->get_id());
    add_task(temp_task);
   }
}
