...
RTA_PFP_RO_NP_SM_OPA::RTA_PFP_RO_NP_SM_OPA()
    : PartitionedSched(false, RTA, FIX_PRIORITY, DPCP, "",
                       "Resource-Oriented") {}

RTA_PFP_RO_NP_SM_OPA::RTA_PFP_RO_NP_SM_OPA(TaskSet tasks,
...
ulong RTA_PFP_RO_NP_SM_OPA::blocking_bound(const Task& ti, uint r_id) {
  ulong bound = 0;
...
ulong RTA_PFP_RO_NP_SM_OPA::request_bound(const Task& ti, uint r_id) {
...
ulong RTA_PFP_RO_NP_SM_OPA::formula_30(const Task& ti, uint p_id,
                                       ulong interval) {
  ulong miu = 0;
...
ulong RTA_PFP_RO_NP_SM_OPA::angent_exec_bound(const Task& ti, uint p_id,
                                              ulong interval) {
  ulong deadline = ti.get_deadline();
  ulong lambda = 0;
  foreach(ti.get_requests(), request) {
...
ulong RTA_PFP_RO_NP_SM_OPA::NCS_workload(const Task& ti, ulong interval) {
...
ulong RTA_PFP_RO_NP_SM_OPA::CS_workload(const Task& ti, uint resource_id,
                                        ulong interval) {
  ulong p = ti.get_period();
  ulong r = ti.get_response_time();
...
ulong RTA_PFP_RO_NP_SM_OPA::response_time_UA(const Task& ti) {
...
ulong RTA_PFP_RO_NP_SM_OPA::response_time_AP(const Task& ti) {
...
ulong RTA_PFP_RO_NP_SM_OPA::response_time_SP(const Task& ti) {
  uint p_id = ti.get_partition();
  ulong test_bound = ti.get_deadline();
  ulong test_start = ti.get_wcet_non_critical_sections();
  ulong response_time = test_start;
...
bool RTA_PFP_RO_NP_SM_OPA::worst_fit_for_resources(uint active_processor_num) {
  /*
...
bool RTA_PFP_RO_NP_SM_OPA::is_first_fit_for_tasks_schedulable(
...
bool RTA_PFP_RO_NP_SM_OPA::alloc_schedulable() { return true; }

bool RTA_PFP_RO_NP_SM_OPA::alloc_schedulable(Task* ti) {
  uint p_id = ti->get_partition();
...
int RTA_PFP_RO_NP_SM_OPA::ROP_SM_Order(Task t1, Task t2) {
  int64_t slack1 = t1.get_deadline();
  int64_t slack2 = t2.get_deadline();
  slack1 -= t1.get_wcet() + t1.get_other_attr();
  slack2 -= t2.get_wcet() + t2.get_other_attr();
  return slack1 < slack2;
}

bool RTA_PFP_RO_NP_SM_OPA::is_schedulable() {
  ...
  for (uint i = 1; i <= p_num; i++) {
    ...
    for (int t = 0; t < taskset.size(); t++) taskset[t].set_index(t);
