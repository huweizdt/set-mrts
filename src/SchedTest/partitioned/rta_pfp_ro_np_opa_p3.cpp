...
#include <rta_pfp_ro_np_opa_p3.h>

using std::min;

RTA_PFP_RO_NP_OPA_P3::RTA_PFP_RO_NP_OPA_P3(TaskSet tasks,
...
RTA_PFP_RO_NP_OPA_P3::~RTA_PFP_RO_NP_OPA_P3() {}

ulong RTA_PFP_RO_NP_OPA_P3::blocking_bound(const Task& ti, uint r_id) {
  ulong bound = 0;
  const Request& request_q = ti.get_request_by_id(r_id);
  Resource& resource_q = resources.get_resource_by_id(r_id);
  for (uint l = tasks.get_taskset_size() - 1; l > ti.get_index(); l--) {
    Task& tl = tasks.get_tasks()[l];
    foreach(tl.get_requests(), request_v) {
      if (r_id == request_v->get_resource_id()) {
...
ulong RTA_PFP_RO_NP_OPA_P3::request_bound(const Task& ti, uint r_id) {
  ...
  while (bound <= deadline) {
    ...
    for (uint h = 0; h < ti.get_index(); h++) {
      Task& th = tasks.get_tasks()[h];
...
ulong RTA_PFP_RO_NP_OPA_P3::formula_30(const Task& ti, uint p_id,
                                       ulong interval) {
  ulong miu = 0;
...
ulong RTA_PFP_RO_NP_OPA_P3::angent_exec_bound(const Task& ti, uint p_id,
                                              ulong interval) {
  ulong deadline = ti.get_deadline();
  ulong lambda = 0;
  foreach(ti.get_requests(), request) {
...
ulong RTA_PFP_RO_NP_OPA_P3::NCS_workload(const Task& ti, ulong interval) {
  ulong e = ti.get_wcet_non_critical_sections();
  ulong p = ti.get_period();
  ulong d = ti.get_deadline();
  return ceiling((interval + d - e), p) * e;
}

ulong RTA_PFP_RO_NP_OPA_P3::CS_workload(const Task& ti, uint resource_id,
                                        ulong interval) {
  ulong p = ti.get_period();
  ulong d = ti.get_deadline();
  const Request& request = ti.get_request_by_id(resource_id);
  ulong agent_length = request.get_num_requests() * request.get_max_length();
  return ceiling((interval + d - agent_length), p) * agent_length;
}

ulong RTA_PFP_RO_NP_OPA_P3::response_time_UA(const Task& ti) {
...
ulong RTA_PFP_RO_NP_OPA_P3::response_time_AP(const Task& ti) {
...
ulong RTA_PFP_RO_NP_OPA_P3::response_time_SP(const Task& ti) {
  uint p_id = ti.get_partition();
  ulong test_bound = ti.get_deadline();
  ulong test_start = ti.get_wcet_non_critical_sections();
  ulong response_time = test_start;
...
bool RTA_PFP_RO_NP_OPA_P3::worst_fit_for_resources(uint active_processor_num) {
...
bool RTA_PFP_RO_NP_OPA_P3::is_first_fit_for_tasks_schedulable(
...
bool RTA_PFP_RO_NP_OPA_P3::OPA(uint p_id) {
  set<uint> unassigned_queue;

  foreach(tasks.get_tasks(), ti) {
    if (ti->get_partition() == p_id) unassigned_queue.insert(ti->get_id());
  }
...
bool RTA_PFP_RO_NP_OPA_P3::alloc_schedulable() { return true; }

bool RTA_PFP_RO_NP_OPA_P3::alloc_schedulable(Task* ti) {
  uint p_id = ti->get_partition();
...
bool RTA_PFP_RO_NP_OPA_P3::is_schedulable() {
