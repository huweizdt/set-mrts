...
RTA_PFP_RO_SM::RTA_PFP_RO_SM(TaskSet tasks, ProcessorSet processors,
...
ulong RTA_PFP_RO_SM::blocking_bound(const Task& ti, uint r_id) {
  ulong bound = 0;
...
ulong RTA_PFP_RO_SM::request_bound(const Task& ti, uint r_id) {
...
ulong RTA_PFP_RO_SM::formula_30(const Task& ti, uint p_id, ulong interval) {
  ulong miu = 0;
...
ulong RTA_PFP_RO_SM::angent_exec_bound(const Task& ti, uint p_id,
                                       ulong interval) {
  ulong deadline = ti.get_deadline();
  ulong lambda = 0;
  foreach(ti.get_requests(), request) {
...
ulong RTA_PFP_RO_SM::NCS_workload(const Task& ti, ulong interval) {
...
ulong RTA_PFP_RO_SM::CS_workload(const Task& ti, uint resource_id,
                                 ulong interval) {
  ulong p = ti.get_period();
  ulong r = ti.get_response_time();
...
ulong RTA_PFP_RO_SM::response_time_AP(const Task& ti) {
...
ulong RTA_PFP_RO_SM::response_time_SP(const Task& ti) {
  uint p_id = ti.get_partition();
  ulong test_bound = ti.get_deadline();
  ulong test_start = ti.get_wcet_non_critical_sections();
  ulong response_time = test_start;
...
bool RTA_PFP_RO_SM::is_schedulable() {
  ...
  for (uint i = 1; i <= p_num; i++) {
    ...
    foreach(tasks.get_tasks(), task)
    {
            ...
            cout<<"Task:"<<task->get_id()<<" priority:"<<task->get_priority()<<"
    Slack:"<<slack<<endl; cout<<"----------------------------"<<endl;
    }
    */
