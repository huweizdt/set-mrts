...
#include <rta_pfp_ro_np_opa_p2.h>

using std::min;

RTA_PFP_RO_NP_OPA_P2::RTA_PFP_RO_NP_OPA_P2(TaskSet tasks,
...
RTA_PFP_RO_NP_OPA_P2::~RTA_PFP_RO_NP_OPA_P2() {}

ulong RTA_PFP_RO_NP_OPA_P2::blocking_bound(const Task& ti, uint r_id) {
  ulong bound = 0;
...
ulong RTA_PFP_RO_NP_OPA_P2::request_bound(const Task& ti, uint r_id) {
...
ulong RTA_PFP_RO_NP_OPA_P2::formula_30(const Task& ti, uint p_id,
                                       ulong interval) {
  ulong miu = 0;
...
ulong RTA_PFP_RO_NP_OPA_P2::angent_exec_bound(const Task& ti, uint p_id,
                                              ulong interval) {
  ulong deadline = ti.get_deadline();
  ulong lambda = 0;
  foreach(ti.get_requests(), request) {
...
ulong RTA_PFP_RO_NP_OPA_P2::NCS_workload(const Task& ti, ulong interval) {
...
ulong RTA_PFP_RO_NP_OPA_P2::CS_workload(const Task& ti, uint resource_id,
                                        ulong interval) {
  ulong p = ti.get_period();
  ulong d = ti.get_deadline();
...
ulong RTA_PFP_RO_NP_OPA_P2::response_time_UA(const Task& ti) {
...
ulong RTA_PFP_RO_NP_OPA_P2::response_time_AP(const Task& ti) {
...
ulong RTA_PFP_RO_NP_OPA_P2::response_time_SP(const Task& ti) {
  uint p_id = ti.get_partition();
  ulong test_bound = ti.get_deadline();
  ulong test_start = ti.get_wcet_non_critical_sections();
  ulong response_time = test_start;
...
bool RTA_PFP_RO_NP_OPA_P2::worst_fit_for_resources(uint active_processor_num) {
...
bool RTA_PFP_RO_NP_OPA_P2::is_first_fit_for_tasks_schedulable(
...
bool RTA_PFP_RO_NP_OPA_P2::OPA(uint p_id) {
...
bool RTA_PFP_RO_NP_OPA_P2::alloc_schedulable() { return true; }

bool RTA_PFP_RO_NP_OPA_P2::alloc_schedulable(Task* ti) {
  uint p_id = ti->get_partition();
...
bool RTA_PFP_RO_NP_OPA_P2::is_schedulable() {
