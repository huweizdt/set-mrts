...
RTA_PFP_ROP_HETEROGENEOUS::RTA_PFP_ROP_HETEROGENEOUS()
    : PartitionedSched(false, RTA, FIX_PRIORITY, DPCP, "",
                       "Resource-Oriented") {}

RTA_PFP_ROP_HETEROGENEOUS::RTA_PFP_ROP_HETEROGENEOUS(TaskSet tasks,
...
double RTA_PFP_ROP_HETEROGENEOUS::get_total_power() {
  return processors.get_total_power_heterogeneous();
}

ulong RTA_PFP_ROP_HETEROGENEOUS::blocking_bound(const Task& ti, uint r_id) {
  ulong bound = 0;
...
ulong RTA_PFP_ROP_HETEROGENEOUS::request_bound(const Task& ti, uint r_id) {
...
ulong RTA_PFP_ROP_HETEROGENEOUS::formula_30(const Task& ti, uint p_id,
                                            ulong interval) {
  ulong miu = 0;
...
ulong RTA_PFP_ROP_HETEROGENEOUS::angent_exec_bound(const Task& ti, uint p_id,
                                                   ulong interval) {
  ulong deadline = ti.get_deadline();
  ulong lambda = 0;
  foreach(ti.get_requests(), request) {
...
ulong RTA_PFP_ROP_HETEROGENEOUS::NCS_workload(const Task& ti, ulong interval) {
...
ulong RTA_PFP_ROP_HETEROGENEOUS::CS_workload(const Task& ti, uint resource_id,
                                             ulong interval) {
  ulong p = ti.get_period();
  ulong r = ti.get_response_time();
...
ulong RTA_PFP_ROP_HETEROGENEOUS::response_time_AP(const Task& ti) {
...
ulong RTA_PFP_ROP_HETEROGENEOUS::response_time_SP(const Task& ti) {
  uint p_id = ti.get_partition();
  ulong test_bound = ti.get_deadline();
  ulong test_start = ti.get_wcet_non_critical_sections_heterogeneous();
  ulong response_time = test_start;
...
bool RTA_PFP_ROP_HETEROGENEOUS::worst_fit_for_resources(
...
bool RTA_PFP_ROP_HETEROGENEOUS::is_first_fit_for_tasks_schedulable(
...
bool RTA_PFP_ROP_HETEROGENEOUS::alloc_schedulable() {
...
bool RTA_PFP_ROP_HETEROGENEOUS::alloc_schedulable(Task* ti) {
  uint p_id = ti->get_partition();
...
bool RTA_PFP_ROP_HETEROGENEOUS::is_schedulable() {
