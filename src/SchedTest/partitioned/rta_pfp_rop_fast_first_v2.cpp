...
#include <rta_pfp_rop_fast_first_v2.h>

using std::min;

RTA_PFP_ROP_FAST_FIRST_V2::RTA_PFP_ROP_FAST_FIRST_V2()
    : PartitionedSched(false, RTA, FIX_PRIORITY, DPCP, "",
                       "Resource-Oriented") {}

RTA_PFP_ROP_FAST_FIRST_V2::RTA_PFP_ROP_FAST_FIRST_V2(TaskSet tasks,
  ...
  this->processors.sort_by_speedfactor(DECREASE);
  // ROP_FF_P++;
  /*	foreach(this->processors.get_processors(), processor)
                  cout<<"processor
     speedupfactor:"<<processor->get_speedfactor()<<endl;*/
}

RTA_PFP_ROP_FAST_FIRST_V2::~RTA_PFP_ROP_FAST_FIRST_V2() {}

double RTA_PFP_ROP_FAST_FIRST_V2::get_total_power() {
  return processors.get_total_power_heterogeneous();
}

ulong RTA_PFP_ROP_FAST_FIRST_V2::blocking_bound(const Task& ti, uint r_id) {
  ulong bound = 0;
...
ulong RTA_PFP_ROP_FAST_FIRST_V2::request_bound(const Task& ti, uint r_id) {
...
ulong RTA_PFP_ROP_FAST_FIRST_V2::formula_30(const Task& ti, uint p_id,
                                            ulong interval) {
  ulong miu = 0;
...
ulong RTA_PFP_ROP_FAST_FIRST_V2::angent_exec_bound(const Task& ti, uint p_id,
                                                   ulong interval) {
  ulong deadline = ti.get_deadline();
  ulong lambda = 0;
  foreach(ti.get_requests(), request) {
...
ulong RTA_PFP_ROP_FAST_FIRST_V2::NCS_workload(const Task& ti, ulong interval) {
...
ulong RTA_PFP_ROP_FAST_FIRST_V2::CS_workload(const Task& ti, uint resource_id,
                                             ulong interval) {
  ulong p = ti.get_period();
  ulong r = ti.get_response_time();
...
ulong RTA_PFP_ROP_FAST_FIRST_V2::response_time_AP(const Task& ti) {
  ...
  while (response_time <= test_bound) {
    ...
    // cout<<"t"<<ti.get_id()<<": wcet:"<<ti.get_wcet_heterogeneous()<<"
...
ulong RTA_PFP_ROP_FAST_FIRST_V2::response_time_SP(const Task& ti) {
  uint p_id = ti.get_partition();
  ulong test_bound = ti.get_deadline();
  ulong test_start = ti.get_wcet_non_critical_sections_heterogeneous();
  ulong response_time = test_start;
...
bool RTA_PFP_ROP_FAST_FIRST_V2::worst_fit_for_resources(
...
bool RTA_PFP_ROP_FAST_FIRST_V2::is_first_fit_for_tasks_schedulable(
...
bool RTA_PFP_ROP_FAST_FIRST_V2::is_worst_fit_for_tasks_schedulable() {
  bool schedulable;
  uint p_num = processors.get_processor_num();
  // tasks.RM_Order();
  ...
  foreach(tasks.get_tasks(), ti) {
    ...
    foreach(processors.get_processors(), p)
      cout << "Processor:" << p->get_processor_id()
           << " utilization:" << p->get_utilization() << endl;
...
bool RTA_PFP_ROP_FAST_FIRST_V2::alloc_schedulable() {
...
bool RTA_PFP_ROP_FAST_FIRST_V2::alloc_schedulable(Task* ti) {
  uint p_id = ti->get_partition();
...
bool RTA_PFP_ROP_FAST_FIRST_V2::is_schedulable() {
  ...
  if (0 != resources.get_resourceset_size()) {
    for (uint i = 1; i <= p_num; i++) {
      for (uint j = 0; j < p_num; j++) {
        ...
        schedulable = is_first_fit_for_tasks_schedulable(j);
        // schedulable = is_worst_fit_for_tasks_schedulable();
        if (schedulable) {
