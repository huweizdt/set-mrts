...
#include <math.h>
#include <rta_pfp_rop_fast_first.h>

using std::min;

RTA_PFP_ROP_FAST_FIRST::RTA_PFP_ROP_FAST_FIRST()
    : PartitionedSched(false, RTA, FIX_PRIORITY, DPCP, "",
                       "Resource-Oriented") {}

RTA_PFP_ROP_FAST_FIRST::RTA_PFP_ROP_FAST_FIRST(TaskSet tasks,
...
double RTA_PFP_ROP_FAST_FIRST::get_total_power() {
  return processors.get_total_power_heterogeneous();
}

ulong RTA_PFP_ROP_FAST_FIRST::blocking_bound(const Task& ti, uint r_id) {
  ulong bound = 0;
...
ulong RTA_PFP_ROP_FAST_FIRST::request_bound(const Task& ti, uint r_id) {
...
ulong RTA_PFP_ROP_FAST_FIRST::formula_30(const Task& ti, uint p_id,
                                         ulong interval) {
  ulong miu = 0;
...
ulong RTA_PFP_ROP_FAST_FIRST::angent_exec_bound(const Task& ti, uint p_id,
                                                ulong interval) {
  ulong deadline = ti.get_deadline();
  ulong lambda = 0;
  foreach(ti.get_requests(), request) {
...
ulong RTA_PFP_ROP_FAST_FIRST::NCS_workload(const Task& ti, ulong interval) {
...
ulong RTA_PFP_ROP_FAST_FIRST::CS_workload(const Task& ti, uint resource_id,
                                          ulong interval) {
  ulong p = ti.get_period();
  ulong r = ti.get_response_time();
...
ulong RTA_PFP_ROP_FAST_FIRST::response_time_AP(const Task& ti) {
...
ulong RTA_PFP_ROP_FAST_FIRST::response_time_SP(const Task& ti) {
  uint p_id = ti.get_partition();
  ulong test_bound = ti.get_deadline();
  ulong test_start = ti.get_wcet_non_critical_sections_heterogeneous();
  ulong response_time = test_start;
...
bool RTA_PFP_ROP_FAST_FIRST::worst_fit_for_resources(
...
bool RTA_PFP_ROP_FAST_FIRST::is_first_fit_for_tasks_schedulable(
...
bool RTA_PFP_ROP_FAST_FIRST::is_worst_fit_for_tasks_schedulable() {
  bool schedulable;
  uint p_num = processors.get_processor_num();
  // tasks.RM_Order();
  foreach(tasks.get_tasks(), ti) {
...
bool RTA_PFP_ROP_FAST_FIRST::alloc_schedulable() {
...
bool RTA_PFP_ROP_FAST_FIRST::alloc_schedulable(Task* ti) {
  uint p_id = ti->get_partition();
...
bool RTA_PFP_ROP_FAST_FIRST::is_schedulable() {
