...
RTA_PFP_RO_NP_SM::RTA_PFP_RO_NP_SM()
    : PartitionedSched(false, RTA, FIX_PRIORITY, DPCP, "",
                       "Resource-Oriented") {}

RTA_PFP_RO_NP_SM::RTA_PFP_RO_NP_SM(TaskSet tasks, ProcessorSet processors,
...
ulong RTA_PFP_RO_NP_SM::blocking_bound(const Task& ti, uint r_id) {
  ulong bound = 0;
...
ulong RTA_PFP_RO_NP_SM::request_bound(const Task& ti, uint r_id) {
...
ulong RTA_PFP_RO_NP_SM::formula_30(const Task& ti, uint p_id, ulong interval) {
  ulong miu = 0;
...
ulong RTA_PFP_RO_NP_SM::angent_exec_bound(const Task& ti, uint p_id,
                                          ulong interval) {
  ulong deadline = ti.get_deadline();
  ulong lambda = 0;
  foreach(ti.get_requests(), request) {
...
ulong RTA_PFP_RO_NP_SM::NCS_workload(const Task& ti, ulong interval) {
...
ulong RTA_PFP_RO_NP_SM::CS_workload(const Task& ti, uint resource_id,
                                    ulong interval) {
  ulong p = ti.get_period();
  ulong r = ti.get_response_time();
...
ulong RTA_PFP_RO_NP_SM::response_time_AP(const Task& ti) {
...
ulong RTA_PFP_RO_NP_SM::response_time_SP(const Task& ti) {
  uint p_id = ti.get_partition();
  ulong test_bound = ti.get_deadline();
  ulong test_start = ti.get_wcet_non_critical_sections();
  ulong response_time = test_start;
...
bool RTA_PFP_RO_NP_SM::worst_fit_for_resources(uint active_processor_num) {
  /*
...
bool RTA_PFP_RO_NP_SM::is_first_fit_for_tasks_schedulable(
...
bool RTA_PFP_RO_NP_SM::alloc_schedulable() {
...
bool RTA_PFP_RO_NP_SM::alloc_schedulable(Task* ti) {
  uint p_id = ti->get_partition();
...
int RTA_PFP_RO_NP_SM::ROP_SM_Order(Task t1, Task t2) {
...
bool RTA_PFP_RO_NP_SM::is_schedulable() {
