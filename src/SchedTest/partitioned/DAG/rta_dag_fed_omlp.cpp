...
RTA_DAG_FED_OMLP::RTA_DAG_FED_OMLP()
    : PartitionedSched(false, RTA, FIX_PRIORITY, OTHER, "", "") {}

RTA_DAG_FED_OMLP::RTA_DAG_FED_OMLP(DAG_TaskSet tasks,
...
bool RTA_DAG_FED_OMLP::alloc_schedulable() {}

// Algorithm 1 Scheduling Test
bool RTA_DAG_FED_OMLP::is_schedulable() {
...
uint64_t RTA_DAG_FED_OMLP::intra_task_blocking(const DAG_Task& ti,
  ...
  alpha = min(m_i, N_i_q);
  if (0 == request_num) {
    return L_i_q * ((alpha*(alpha-1))/2 + (m_i-1) * (N_i_q-alpha));
  } else {
...
uint32_t RTA_DAG_FED_OMLP::maximum_request_number(const DAG_Task& ti,
                                                        const DAG_Task& tj,
                                                        uint32_t res_id) {
...
uint64_t RTA_DAG_FED_OMLP::inter_task_blocking(const DAG_Task& ti, uint32_t res_id, uint32_t request_num) {
  uint32_t N_i_q, m_i;
...
uint64_t RTA_DAG_FED_OMLP::total_blocking_time(const DAG_Task& ti) {
  uint64_t sum = 0;
...
uint64_t RTA_DAG_FED_OMLP::get_response_time(const DAG_Task& ti) {
