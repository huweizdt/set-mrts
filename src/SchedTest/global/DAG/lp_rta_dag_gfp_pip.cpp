...
/** Class PIPMapper */
uint64_t DAG_PIPMapper::encode_request(uint64_t task_id, uint64_t res_id,
...
uint64_t DAG_PIPMapper::get_type(uint64_t var) {
  return (var >> 30) & (uint64_t)0x7;  // 3 bits
}

uint64_t DAG_PIPMapper::get_task(uint64_t var) {
  return (var >> 20) & (uint64_t)0x3ff;  // 10 bits
}

uint64_t DAG_PIPMapper::get_res_id(uint64_t var) {
  return (var >> 16) & (uint64_t)0xf;  // 4 bits
}

uint64_t DAG_PIPMapper::get_req_id(uint64_t var) {
  return var & (uint64_t)0xffff;  // 16 bits
}

DAG_PIPMapper::DAG_PIPMapper(uint start_var) : VarMapperBase(start_var) {}

uint DAG_PIPMapper::lookup(uint task_id, uint res_id, uint req_id,
                           var_type type) {
...
string DAG_PIPMapper::key2str(uint64_t key, uint var) const {
  ostringstream buf;

  switch (get_type(key)) {
    case DAG_PIPMapper::BLOCKING_DIRECT:
      buf << "Xd[";
      break;
    case DAG_PIPMapper::BLOCKING_INDIRECT:
      buf << "Xi[";
      break;
    case DAG_PIPMapper::BLOCKING_PREEMPT:
      buf << "Xp[";
      break;
    case DAG_PIPMapper::BLOCKING_OTHER:
      buf << "Xo[";
      break;
    case DAG_PIPMapper::INTERF_REGULAR:
      buf << "Ir[";
      break;
    case DAG_PIPMapper::INTERF_CO_BOOSTING:
      buf << "Ic[";
      break;
    case DAG_PIPMapper::INTERF_STALLING:
      buf << "Is[";
      break;
    case DAG_PIPMapper::INTERF_OTHER:
      buf << "Io[";
      break;
    default:
      buf << "?[";
...
/** Class LP_RTA_DAG_GFP_PIP */

LP_RTA_DAG_GFP_PIP::LP_RTA_DAG_GFP_PIP()
    : GlobalSched(true, RTA, FIX_PRIORITY, PIP, "", "PIP") {}

LP_RTA_DAG_GFP_PIP::LP_RTA_DAG_GFP_PIP(DAG_TaskSet tasks,
...
bool LP_RTA_DAG_GFP_PIP::is_schedulable() {
  bool update = false;
...
ulong LP_RTA_DAG_GFP_PIP::response_time(const DAG_Task& ti) {
  ulong response_t = 0;
...
ulong LP_RTA_DAG_GFP_PIP::workload_bound(const DAG_Task& tx, ulong Ri) {
  ulong e = tx.get_wcet();
...
ulong LP_RTA_DAG_GFP_PIP::holding_bound(const DAG_Task& ti, const DAG_Task& tx,
                                    uint resource_id) {
...
ulong LP_RTA_DAG_GFP_PIP::wait_time_bound(const DAG_Task& ti,
                                          uint resource_id) {
  ulong wait_time = 0;
...
/** Expressions */
void LP_RTA_DAG_GFP_PIP::lp_pip_directed_blocking(const DAG_Task& ti,
                                                  const DAG_Task& tx,
                                                  DAG_PIPMapper* vars,
  ...
  foreach(tx.get_requests(), request) {
    ...
    foreach_request_instance(ti, tx, q, v) {
      ...
      var_id = vars->lookup(x, q, v, DAG_PIPMapper::BLOCKING_DIRECT);
      exp->add_term(var_id, coef * length);
    }
  }
}

void LP_RTA_DAG_GFP_PIP::lp_pip_indirected_blocking(const DAG_Task& ti,
                                                    const DAG_Task& tx,
                                                    DAG_PIPMapper* vars,
                                                    LinearExpression* exp,
                                                    double coef) {
  ...
  foreach(tx.get_requests(), request) {
    ...
    foreach_request_instance(ti, tx, q, v) {
      uint var_id;
      var_id = vars->lookup(x, q, v, DAG_PIPMapper::BLOCKING_INDIRECT);
      exp->add_term(var_id, coef * length);
    }
  }
}

void LP_RTA_DAG_GFP_PIP::lp_pip_preemption_blocking(const DAG_Task& ti,
                                                    const DAG_Task& tx,
                                                    DAG_PIPMapper* vars,
                                                    LinearExpression* exp,
                                                    double coef) {
  ...
  foreach(tx.get_requests(), request) {
    ...
    foreach_request_instance(ti, tx, q, v) {
      ...
      var_id = vars->lookup(x, q, v, DAG_PIPMapper::BLOCKING_PREEMPT);
      exp->add_term(var_id, coef * length);
    }
  }
}

void LP_RTA_DAG_GFP_PIP::lp_pip_OD(const DAG_Task& ti, DAG_PIPMapper* vars,
  ...
  foreach_higher_priority_task(tasks.get_tasks(), ti, th) {
    ...
    var_id = vars->lookup(h, 0, 0, DAG_PIPMapper::INTERF_REGULAR);
  ...
  foreach_lower_priority_task(tasks.get_tasks(), ti, tl) {
    ...
    var_id = vars->lookup(l, 0, 0, DAG_PIPMapper::INTERF_CO_BOOSTING);
    exp->add_term(var_id, coef * (1.0 / p_num));

    var_id = vars->lookup(l, 0, 0, DAG_PIPMapper::INTERF_STALLING);
    exp->add_term(var_id, coef * (1.0 / p_num));
...
void LP_RTA_DAG_GFP_PIP::lp_pip_declare_variable_bounds(const DAG_Task& ti,
                                                        LinearProgram* lp,
                                                        DAG_PIPMapper* vars) {
  foreach_task_except(tasks.get_tasks(), ti, tx) {
    ...
    var_id = vars->lookup(x, 0, 0, DAG_PIPMapper::INTERF_REGULAR);
    lp->declare_variable_bounds(var_id, true, 0, false, -1);

    var_id = vars->lookup(x, 0, 0, DAG_PIPMapper::INTERF_CO_BOOSTING);
    lp->declare_variable_bounds(var_id, true, 0, false, -1);

    var_id = vars->lookup(x, 0, 0, DAG_PIPMapper::INTERF_STALLING);
    lp->declare_variable_bounds(var_id, true, 0, false, -1);
  }
}

void LP_RTA_DAG_GFP_PIP::lp_pip_objective(const DAG_Task& ti, LinearProgram* lp,
                                          DAG_PIPMapper* vars,
...
void LP_RTA_DAG_GFP_PIP::lp_pip_add_constraints(const DAG_Task& ti,
                                                LinearProgram* lp,
                                                DAG_PIPMapper* vars) {
...
void LP_RTA_DAG_GFP_PIP::lp_pip_constraint_1(const DAG_Task& ti,
                                             LinearProgram* lp,
                                             DAG_PIPMapper* vars) {
  foreach_task_except(tasks.get_tasks(), ti, tx) {
...
void LP_RTA_DAG_GFP_PIP::lp_pip_constraint_2(const DAG_Task& ti,
                                             LinearProgram* lp,
                                             DAG_PIPMapper* vars) {
  foreach_task_except(tasks.get_tasks(), ti, tx) {
    LinearExpression* exp = new LinearExpression();
...
void LP_RTA_DAG_GFP_PIP::lp_pip_constraint_3(const DAG_Task& ti,
                                             LinearProgram* lp,
                                             DAG_PIPMapper* vars) {
  foreach_task_except(tasks.get_tasks(), ti, tx) {
    uint x = tx->get_priority();
...
void LP_RTA_DAG_GFP_PIP::lp_pip_constraint_4(const DAG_Task& ti,
                                             LinearProgram* lp,
                                             DAG_PIPMapper* vars) {
...
void LP_RTA_DAG_GFP_PIP::lp_pip_constraint_5(const DAG_Task& ti,
                                             LinearProgram* lp,
                                             DAG_PIPMapper* vars) {
  LinearExpression* exp = new LinearExpression();
...
void LP_RTA_DAG_GFP_PIP::lp_pip_constraint_6(const DAG_Task& ti,
                                             LinearProgram* lp,
                                             DAG_PIPMapper* vars) {
  LinearExpression* exp = new LinearExpression();
...
void LP_RTA_DAG_GFP_PIP::lp_pip_constraint_7(const DAG_Task& ti,
                                             LinearProgram* lp,
                                             DAG_PIPMapper* vars) {
  uint p_num = processors.get_processor_num();
...
void LP_RTA_DAG_GFP_PIP::lp_pip_constraint_8(const DAG_Task& ti,
                                             LinearProgram* lp,
                                             DAG_PIPMapper* vars) {
  foreach_lower_priority_task(tasks.get_tasks(), ti, tx) {
    uint x = tx->get_priority();
...
void LP_RTA_DAG_GFP_PIP::lp_pip_constraint_9(const DAG_Task& ti,
                                             LinearProgram* lp,
                                             DAG_PIPMapper* vars) {
...
void LP_RTA_DAG_GFP_PIP::lp_pip_constraint_10(const DAG_Task& ti,
                                              LinearProgram* lp,
                                              DAG_PIPMapper* vars) {
  LinearExpression* exp = new LinearExpression();
...
void LP_RTA_DAG_GFP_PIP::lp_pip_constraint_11(const DAG_Task& ti,
                                              LinearProgram* lp,
                                              DAG_PIPMapper* vars) {
  ulong bound = 0;
