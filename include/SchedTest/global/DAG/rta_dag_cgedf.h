...
#ifndef INCLUDE_SCHEDTEST_GLOBAL_DAG_RTA_DAG_CGEDF_H_
#define INCLUDE_SCHEDTEST_GLOBAL_DAG_RTA_DAG_CGEDF_H_
...
class RTA_DAG_CGEDF : public GlobalSched {
 protected:
  DAG_TaskSet tasks;
 ...
 public:
  RTA_DAG_CGEDF();
  RTA_DAG_CGEDF(DAG_TaskSet tasks, ProcessorSet processors,
                   ResourceSet resources);
  ~RTA_DAG_CGEDF();
  bool is_schedulable();
};

class RTA_DAG_CGEDF_v2 : public RTA_DAG_CGEDF {
 protected:
 ...
 public:
  RTA_DAG_CGEDF_v2();
  RTA_DAG_CGEDF_v2(DAG_TaskSet tasks, ProcessorSet processors,
                 ResourceSet resources);
  ~RTA_DAG_CGEDF_v2();
  bool is_schedulable();
};

class RTA_DAG_CGEDF_v3 : public RTA_DAG_CGEDF_v2 {
 protected:
  uint64_t inter_interference(const DAG_Task& ti, uint64_t interval);
  uint64_t response_time(const DAG_Task& ti);
  uint64_t interf_cal(vector<Interf> Q, const DAG_Task &ti, uint p_num);
 public:
  RTA_DAG_CGEDF_v3();
  RTA_DAG_CGEDF_v3(DAG_TaskSet tasks, ProcessorSet processors,
                 ResourceSet resources);
  ~RTA_DAG_CGEDF_v3();
  bool is_schedulable();
};
#endif  // INCLUDE_SCHEDTEST_GLOBAL_DAG_RTA_DAG_CGEDF_H_
