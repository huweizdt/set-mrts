...
class BPTask {
 ...
 BPTask::BPTask(uint id, ulong wcet, ulong period, ulong deadline,int linkID=0,bool status=false);
/**
 ...
 * Return: NULL
 */
 BPTask::BPTask(uint id, ulong wcet, ulong period, ulong deadline, uint priority,int linkID=0,bool status=false);
 
 uint BPTask::get_linkID() const;
 void BPTask::set_linkID(uint linkID);
 bool BPTask::get_status() const;
 void BPTask::set_status(bool status) ;
